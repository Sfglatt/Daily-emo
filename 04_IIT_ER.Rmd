...
EMA_df <- read.csv("Created_data/EMA_cleaned_SG_2024-09-03.csv")
...
# 2024-09-13 idea
```{r}
# maybe we can do canonical correlations/loadings
# abs value of .5
# ER strategy use as canonical loadings - set 1 = list of ER strategies. for set 2 - symptoms; or two IFIs (P and N like wisteria did); or negative and positive affect changes...so among ER strategies, what are the ones that relate the most to flexibility, or some of the symptoms, or 

# So: 1) categorize (e.g., just look at flexibility); 2) we can look at canonical correlations; 3) simplify! forget about P and N. Just focus on average for each trial (the NP variables I made for everyone); 4) later, if we start to see effects, is it driven by positive/negative? /// if don't see effects, maybe it only applies to positive/negative?
```

```{r canonical correlations}
# Define sets
Set_1_ER <- EMA_df[, c("N1_ER_1", "N1_ER_2", "N1_ER_3", "N1_ER_4", "N1_ER_5", "N1_ER_6", "N1_ER_7")]
Set_2_outcomes <- EMA_df[, c("P_IFI", "N_IFI")]
...
corr <- matcor(Set_1_ER, Set_2_outcomes)
img.matcor(corr, type = 2)

# Canonical correlations
canon <- cc(Set_1_ER, Set_2_outcomes)
# canon$cor
canon      # all results
canon[3:4] # raw canonical coefficients
...
# Canonical loadings
canon_loads <- comput(Set_1_ER, Set_2_outcomes, canon)
canon_loads[3:6] # canonical loadings


# standardized canonical coefficients diagonal matrix of ER
s1 <- diag(sqrt(diag(cov(Set_1_ER))))
s1 %*% canon$xcoef
